...
function openAyarlarMerkezi(){
  let o=document.getElementById('ayarlar-merkezi-overlay');if(!o){o=document.createElement('div');o.id='ayarlar-merkezi-overlay';o.className='modal-overlay';o.style.zIndex='680';document.body.appendChild(o);}const yonetici=_buro?.rol==='yonetici',davet=_buro&&['yonetici','ortak'].includes(_buro.rol),masaustu=!!window.sekreterDesktop;
  ...
  const guvenlik=[yonetici?ayarButonu('🕘 İşlem ve Erişim Günlüğü','openIslemGunlugu()'):'',ayarButonu('↓ Yedek ve Dışa Aktar','openDisaAktar()'),ayarButonu('🔔 Bildirim Ayarları','openBildirimAyarlari()'),ayarButonu('🛡️ KVKK Aydınlatma Metni','kvkkAydinlatmaPenceresi()')].join('');
