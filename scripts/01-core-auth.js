...
function platformKullanicilariCiz(){
  ...
  if(liste.length)tbody.querySelectorAll('tr').forEach((tr,i)=>{const x=liste[i],td=document.createElement('td');td.innerHTML=x.rol==='platform_admini'?'<small style="color:#98a2b3;">Korunuyor</small>':`<button class="btn platform-delete-btn" onclick="platformKullaniciSil('${x.id}','${esc(x.email||'')}')">Hesabı Sil</button>`;tr.appendChild(td);});
}
