...
const AYDINLATMA_METNI_SURUMU='1.0';
...
function platformAdminPaneliniCiz(){
  ...
  const buroKartlari=alan.querySelectorAll('section:first-of-type>div:last-child>div');buroKartlari.forEach((kart,i)=>{const b=burolar[i];if(!b?.id)return;kart.insertAdjacentHTML('beforeend',`<button class="btn platform-delete-btn" onclick="platformBuroSil('${b.id}','${esc(b.ad||'')}')">Büroyu Sil</button>`);});
...
function platformKullanicilariCiz(){
  ...
  if(liste.length)tbody.querySelectorAll('tr').forEach((tr,i)=>{const x=liste[i],td=document.createElement('td');td.innerHTML=x.rol==='platform_admini'?'<small style="color:#98a2b3;">Korunuyor</small>':`<button class="btn platform-delete-btn" onclick="platformKullaniciSil('${x.user_id}','${esc(x.email||'')}')">Hesabı Sil</button>`;tr.appendChild(td);});
}
