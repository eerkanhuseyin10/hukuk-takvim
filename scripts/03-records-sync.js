...
function kayitMuvekkilAramaSonuclari(){
  ...
  box.innerHTML=bulunan.length?bulunan.map(m=>`<button type="button" onclick="kayitMuvekkilSec('${m.id}')"><b>${esc(m.ad)}</b>${m.telefon?`<small>${esc(m.telefon)}</small>`:''}</button>`).join(''):'<div class="empty">Eşleşen kayıt yok. “Yeni Müvekkil” ile ekleyebilirsiniz.</div>';box.style.display='block';
}
...
function kayitEkranindanYeniMuvekkilEkle(){let ov=document.getElementById('kayit-yeni-muvekkil-overlay');if(!ov){ov=document.createElement('div');ov.id='kayit-yeni-muvekkil-overlay';ov.className='modal-overlay';ov.style.zIndex='10020';ov.innerHTML=`<div class="modal" style="max-width:480px;"><div class="modal-header"><h2>Yeni Müvekkil</h2><button type="button" class="btn" onclick="document.getElementById('kayit-yeni-muvekkil-overlay').remove()">✕</button></div><div class="modal-body"><div class="fg"><label>Ad Soyad / Unvan</label><input id="kayit-yeni-m-ad" autocomplete="off"></div><div class="fg"><label>Telefon (opsiyonel)</label><input id="kayit-yeni-m-tel" type="tel"></div><div class="fg"><label>E-posta (opsiyonel)</label><input id="kayit-yeni-m-mail" type="email"></div><button type="button" class="save-btn" onclick="kayitYeniMuvekkilKaydet()">Kaydet ve Seç</button></div></div>`;document.body.appendChild(ov);}ov.style.display='flex';setTimeout(()=>document.getElementById('kayit-yeni-m-ad')?.focus(),30);}
async function kayitYeniMuvekkilKaydet(){const ad=document.getElementById('kayit-yeni-m-ad')?.value.trim(),telefon=document.getElementById('kayit-yeni-m-tel')?.value.trim(),email=document.getElementById('kayit-yeni-m-mail')?.value.trim();if(!ad){alert('Müvekkil adı gereklidir.');return;}const kayit={buro_id:_buro.id,ad,created_by:_user.id};if(telefon)kayit.telefon=telefon;if(email)kayit.email=email;const{data,error}=await sb.from('muvekkiller').insert(kayit).select('*').single();if(error){alert('Müvekkil eklenemedi: '+error.message);return;}MUVEKKIL_KARTLARI.push(data);ST.muvekkilIds=[...(ST.muvekkilIds||[]),String(data.id)];if(ST.davaDosyasiId){const{error:bagHata}=await sb.from('dava_dosyasi_muvekkilleri').insert({dava_dosyasi_id:ST.davaDosyasiId,muvekkil_id:data.id});if(!bagHata)DOSYA_MUVEKKIL_BAGLARI.push({dava_dosyasi_id:ST.davaDosyasiId,muvekkil_id:data.id});}document.getElementById('kayit-yeni-muvekkil-overlay')?.remove();renderKayitKartSecicileri(data.id,ST.davaDosyasiId);}
