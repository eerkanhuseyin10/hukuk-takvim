...
async function uyapIcsAktar(){
  ...
  try{
    ...
    UYAP_ICS_ONIZLEME.forEach(x=>{const k=dKey(x);if(!dMap.has(k)){dMap.set(k,{mahkeme:x.mahkeme,dosya_no:x.dava});yeniD.push({buro_id:_buro.id,mahkeme:x.mahkeme,dosya_no:x.dava,konu:x.dosyaTuru||null,hukuk_alani:uyapIcsDalEtiketi(uyapIcsDal(x.dosyaTuru,x.mahkeme,x.islem)),karsi_taraf:x.diger.map(y=>y.name).join(', ')||null,durum:'acik',created_by:_user.id});}});
